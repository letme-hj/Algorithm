...
{
	t_line* lines = (t_line*) malloc(20*sizeof(t_line));
	(*num_line) = 0;
	...
	for (int i = 0; i < num_point; i++)
	{
		for (int j = (i + 1); j < num_point; j++)
		{
			...
			int c = points[j].y * points[i].x - points[i].y * points[j].x;
			...
			if (a * points[k].x + b * points[k].y - c > 0)
			{
				...
				for (; l < num_point;)
				{
					if (a * points[l].x + b * points[l].y - c < 0)
						break;
					else
					{
						l++;
				...
				{
					(*num_line)++;
					lines[(*num_line) - 1].from = points[i];
					lines[(*num_line) - 1].to = points[j];
				}
			}
